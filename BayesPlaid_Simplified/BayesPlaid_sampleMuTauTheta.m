function plaid = BayesPlaid_sampleMuTauTheta(plaid)
...
if( DEBUG )
    if(mu_theta ~= new_mu_theta)
        display(['BayesPlaid_sampleMuTauTheta: mu_theta changed ', num2str(mu_theta), ...
            ' -> ', num2str(new_mu_theta)]);
    end
    
    if(tau_theta ~= new_tau_theta)
        display(['BayesPlaid_sampleMuTauTheta: tau_theta changed ', num2str(tau_theta), ...
