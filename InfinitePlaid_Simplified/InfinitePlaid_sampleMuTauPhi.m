function plaid = InfinitePlaid_sampleMuTauPhi(plaid)
...
if( DEBUG )
    if(mu_phi ~= new_mu_phi)
        display(['InfinitePlaid_sampleMuTauPhi: mu_phi changed ', num2str(mu_phi), ...
            ' -> ', num2str(new_mu_phi)]);
    end
    
    if(tau_phi ~= new_tau_phi)
        display(['InfinitePlaid_sampleMuTauPhi: tau_phi changed ', num2str(tau_phi), ...
            ' -> ', num2str(new_tau_phi)]);
    end
end
