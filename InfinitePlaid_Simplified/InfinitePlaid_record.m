function recorder = InfinitePlaid_record(plaid, recorder, recitr, Z_file_name,SHOW_FIG)
% recorder = InfinitePlaid_record(plaid, recorder, recitr, Z_file_name, SHOW_FIG)
%
...
if isnan(loglk(recitr)) | isnan(test_loglk(recitr))
   LL
   tau0
   suf_r
   errs
...
if isnan(loglk_marginal(recitr)) | isnan(test_loglk_marginal(recitr))
   LL
   tau0
   suf_r_gibbs
   errs
...
%% Compute averages of Rand index and Omega index if the answer is provided
if ~strcmp(Z_file_name, 'null')
    load(Z_file_name);
    trueK = K;
    K = plaid.KK; % reload
...
%% save the currently instantiated sub-matrices
KK_history(recitr) = plaid.KK;
...
if SHOW_FIG
    ...
    fig = figure(102);
    plot(1:recitr, KK_history(1:recitr));
    title('History of a number of sub-matrices instantiated');
...
recorder.KK_history = KK_history;
